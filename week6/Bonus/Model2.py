...
class ThreeLayerNet():
    ...
    def forward(self, X, y=None):
        ...
        h = np.dot(X, W1) + b1 # Linear 1: (N, H)
        a = np.maximum(0, h) # Activation (Relu)
        h2 = np.dot(a, W2) + b2 # Linear 1: (N, H2)
        a2 = np.maximum(0, h2) # Activation (Relu)
        o = np.dot(a2, W3) + b3 # Linear 3 : (N, C)
